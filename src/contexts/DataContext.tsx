...
// KEY FIX: refreshInventory no longer calls setInventory([]) before the new
// data arrives. It just busts the stale-time cache key and re-fetches.
// The existing data stays visible until the fresh response replaces it.
...
interface DataContextType {
  inventory: any[];
  customers: any[];
  getSales: (businessId: string, period: string, start?: string, end?: string) => Sale[];
  fetchSales: (businessId: string, period: string, start?: string, end?: string) => Promise<void>;
  getRevenueAnalytics: (businessId: string, period: string, start?: string, end?: string) => RevenueAnalytics;
  fetchRevenueAnalytics: (businessId: string, period: string, start?: string, end?: string) => Promise<void>;
...
export const DataProvider = ({ children }: any) => {
    ...
    try {
      ...
      if (shouldNotify) {
        ...
        if (outOfStock.length > 0) {
          ...
          addNotification('error', `${outOfStock.length} item${outOfStock.length > 1 ? 's' : ''} out of stock`, `${names}${extra} need restocking immediately.`);
        }

        if (lowStock.length > 0) {
          const names = lowStock.slice(0, 3).map((i: any) => i.name).join(', ');
          const extra = lowStock.length > 3 ? ` and ${lowStock.length - 3} more` : '';
          addNotification('warning', `${lowStock.length} item${lowStock.length > 1 ? 's' : ''} running low`, `${names}${extra} are below their stock threshold.`);
        }
  ...
  const fetchCustomers = async (businessId: string) => {
    ...
    setFetchingKeys(prev => ({ ...prev, [key]: true }));
    try {
      const res = await apiFetch(`https://api.aflows.uk/api/v1/customers?businessId=${businessId}`);
      const data = await res.json();
  ...
  const fetchBusiness = async (businessId: string) => {
    const res = await apiFetch(`https://api.aflows.uk/api/v1/business/settings?businessId=${businessId}`);
    const data = await res.json();
    const result = Array.isArray(data) ? data[0] : data;
    if (result?.success) setBusiness(result);
  };

  // ─── SALES ────────────────────────────────────────────────────────────────
  const fetchSales = async (businessId: string, period: string = "today", start?: string, end?: string) => {
    ...
    const promise = (async () => {
      ...
      try {
        ...
        setSalesCache((prev) => ({ ...prev, [key]: mapped }));
        setLastFetched((prev) => ({ ...prev, [key]: Date.now() }));
      } catch (err) {
  ...
  // ─── ANALYTICS ────────────────────────────────────────────────────────────
  const fetchRevenueAnalytics = async (businessId: string, period: string = "month", start?: string, end?: string) => {
    const key = getKey(businessId, period, start, end);
  ...
  // ─── GETTERS ──────────────────────────────────────────────────────────────
  const getSales = (businessId: string, period: string, start?: string, end?: string) => {
    const key = getKey(businessId, period, start, end);
    return salesCache[key] || [];
  };

  const getRevenueAnalytics = (businessId: string, period: string, start?: string, end?: string): RevenueAnalytics => {
    const key = getKey(businessId, period, start, end);
  ...
  const refreshSales = async (businessId: string, period: string, start?: string, end?: string) => {
    const key = getKey(businessId, period, start, end);
    setSalesCache((prev) => { const next = { ...prev }; delete next[key]; return next; });
    setLastFetched((prev) => { const next = { ...prev }; delete next[key]; return next; });
    await fetchSales(businessId, period, start, end);
  };

  const refreshBusiness = async (businessId: string) => await fetchBusiness(businessId);
  const refreshCustomers = async (businessId: string) => await fetchCustomers(businessId);
  ...
  const updateSaleReceipt = (businessId: string, saleId: string, receipt_id: string, receipt_number: string) => {
    setSalesCache(prev =>
      Object.fromEntries(
        Object.entries(prev).map(([k, v]) => [
          k,
          (v as any[]).map(s =>
            s.id === saleId ? { ...s, receipt_id, receipt_number, _receiptPending: false } : s
          ),
        ])
      )
    );
