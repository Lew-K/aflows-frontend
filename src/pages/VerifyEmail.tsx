...
import { CheckCircle2, AlertCircle, Mail, InfoIcon } from 'lucide-react';

const VerifyEmail = () => {
  const [searchParams] = useSearchParams();
  const navigate = useNavigate();
  const { isAuthenticated } = useAuth();
  const [state, setState] = useState<'loading' | 'success' | 'error' | 'already-verified'>('loading');
    ...
    const verify = async () => {
      try {
        ...
        if (errorMessage.includes('expired') || errorMessage.includes('Invalid or expired')) {
          setState('error');
          setMessage('This verification link has expired. Please request a new one from your email or contact support.');
        } else if (errorMessage.includes('already') || errorMessage.includes('verified')) {
          setState('already-verified');
          setMessage('This email has already been verified. You can proceed to your dashboard.');
        } else {
          setState('error');
  ...
  if (isAuthenticated && state === 'already-verified') {
    return (
        ...
        >
          ...
          >
            ...
            <div>
              ...
              <p className="text-muted-foreground text-sm">Your email has already been verified. You're all set!</p>
            </div>
            <Button
              variant="hero"
